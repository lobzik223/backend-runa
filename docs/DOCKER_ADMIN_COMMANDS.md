...
## 1. Один раз: создать таблицу админов и сгенерировать Prisma-клиент

После деплоя кода с моделью `Admin`:

```bash
docker-compose -f docker-compose.prod.yml exec backend npx prisma migrate deploy
```

(или `docker-compose exec backend npx prisma migrate deploy` без prod-файла)

Если миграции не используете, а только `db push`:
...
## 4. Удалить всех пользователей из БД
...
## 6. Снять Premium у пользователя
